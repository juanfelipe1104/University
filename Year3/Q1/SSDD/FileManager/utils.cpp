...
void packString(std::vector<unsigned char> &packet, std::string data)
{
    pack(packet, (int)data.size());
    packv(packet, (char *)data.data(), data.size());
}
std::string unpackString(std::vector<unsigned char> &packet)
{
    std::string string;
    string.resize(unpack<int>(packet));
    unpackv(packet, (char *)string.data(), string.size());
    return string;
}