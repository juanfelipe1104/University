...
object key_value_rdds extends App {
  ...
  val createAccumulator2 = (salary: Int) => salary
  val addValue2 = (accumulator: Int, salary: Int) => Math.max(accumulator, salary)
  val mergeAccumulators2 = (accumulator1: Int, accumulator2: Int) => Math.max(accumulator1, accumulator2)
  val result2 = salarios.combineByKey(createAccumulator2, addValue2, mergeAccumulators2)
  ...
  val filter_rdd2 = my_rdd.groupByKey().filter(x => x._2.size >= 2).flatMap(x => x._2.map((x._1, _)))
