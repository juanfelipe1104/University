...
object sparkSQL extends App {
  ...
  val fruitsFilename = "src/main/scala/files/fruits.json"
  val df = spark.read.json(fruitsFilename)
  ...
  val df2 = rdd.map { case (k, v) => KV(k, v) }.toDF()

  // De Dataframe a tabla SQL
  df2.createOrReplaceTempView("KeyValue")
  ...
  val accessLogFilename = "src/main/scala/files/access.log"
  val lines = sc.textFile(accessLogFilename, 10).map(_.replaceAll("\\[", "\"").replaceAll("\\]", "\"")).map(line => line.split("\"").map(_.trim).filter(_.nonEmpty)).filter(_.size > 6)
  ...
  val df3 = spark.createDataFrame(rows)
  df3.agg(max("status")).show
  ...
  val emailsFilename = "src/main/scala/files/emails.txt"
  val emailsRDD = sc.textFile(emailsFilename)
  ...
  //Ejercicio: UDF
  spark.udf.register("extractdomain", (email: String) => {
    email.dropWhile(c => c != '@').stripPrefix("@")
  })
  spark.sql("SELECT extractdomain(para) FROM emails").show

  //Ejercicio 2: UDF extractyear para extraer año de la fecha
  spark.udf.register("extractyear", (fecha: String) => {
  ...
  spark.sql("CREATE TABLE winemagcsv USING csv OPTIONS (path 'src/main/scala/files/winemag.csv', header 'true', inferSchema 'true')")
  ...
  spark.sql("CREATE TABLE winemag_quotes USING csv OPTIONS (path 'src/main/scala/files/winemag_special_format.csv', header 'true', inferSchema 'true', quote '$', sep '|', comment '-')")
  ...
  spark.sql("CREATE TABLE jsontable USING json OPTIONS (path 'src/main/scala/files/earthquakes_2024.json')")
